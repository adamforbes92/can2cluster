...
// FreeRTOS task and communicating with canReceiveTask via a dedicated queue.
//
// TP2.0 (KWP2000 over VW TP2.0)
//   - Connects to DSG gearbox (module 0x02) on VW MK5 / PQ35 platforms
//   - Protocol confirmed from VCDS SavvyCAN capture
...
//   - Configurable TX/RX CAN IDs and DID via can2cluster_uds.h defines
// =============================================================================

#include "can2cluster_uds.h"
#include "can2cluster_defs.h"

// Shared queues (created in the task init; populated by canReceiveTask)
...
// Send a KWP2000 payload as a TP2.0 single-data-frame on TP20_TX_ID.
...
static bool tp20Connect()
{
    ...
    // Byte layout confirmed from VCDS SavvyCAN log:
...
void taskTP20(void *arg)
{
    ...
    // from canReceiveTask in can2cluster_can.cpp
    ...
    while (1) {
        ...
        while (useTP20 && hasCAN) {
            ...
            dsgUDSSpeed = tp20Speed; // keep legacy alias in sync

            DEBUG("[TP2.0] raw=%u  speed=%u km/h", raw, tp20Speed);
        }
...
// =============================================================================
// UDS TASK  (ISO-TP / ISO 14229 pattern, queue-based)
// =============================================================================
    ...
    while (1) {
        ...
        while (useUDS && hasCAN) {
            ...
            for (;;) {
                ...
                DEBUG("[UDS] DID 0x%04X  speed=%u km/h", respDID, udsSpeed);
                break;
            }
        }
